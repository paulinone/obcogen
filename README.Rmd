...
The goal of obcogen (object code generator) is to create an ability to create code that would build a data.frames or tibbles. The idea came about when creating tests and I was expecting an equal tibble, but I did not want to type out 20 observations for 5 variables. Imagine if it was bigger...
...
I failed to realize that species was a factor and `hr` and `hr1` were not the same, but `hr` and `hr2` are the same.
 
And just a final comparison with the original idea, does `head(iris)` equal what you expect it to. We will test with both the `waldo` and `testthat` packages.
```{r}
...
One caveat, I am hoping that I find solution to. I have noticed that R can only have so characters in a line of code. If you put `code_gen(iris)`. You will get a whole string, but if you paste it directly R will not evaluate it properly. I found out I had to create a new line of code for the "Species" variable. It's annoying, but still quicker than typing out the whole `iris` dataset.
...
I sincerely hope this helps someone, please let me know if you find any bugs!
